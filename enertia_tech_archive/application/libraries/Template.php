...
class Template {
   ...
   var $parse_template = FALSE;
    ...
    public function  __construct() {
      ...
      if (isset($template))
      {
      //print_R($template['active_template']);die;
   ...
   {
       // echo $group.' '.$template.' '.$activate;die;
      if ( ! isset($this->config[$group]))
      {
   ...
   */
   ...
   function write_view($region, $view, $data = NULL, $overwrite = FALSE)
   {
     ...
     // print_R($view);   print_R($data); 
      $content = $this->CI->load->view($view, $data, TRUE);
  //print_R($overwrite);  echo '  '; print_R($region); echo '  ';  print_R($content);die;
   ...
   */
   ...
   function render($region = NULL, $buffer = FALSE, $parse = FALSE)
   {
      ...
      else
      {
       ...
       // echo $this->parse_template;   echo $parse;  die;
